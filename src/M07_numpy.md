...
%% md prov-12
El `shape` acá es `(2, 3)`: dos filas y tres columnas. En el curso vamos a trabajar casi siempre con arrays **1D** (señales en el tiempo), así que no vamos a profundizar en 2D por ahora. Aparecerán de forma natural cuando, en algún lab, guardemos varias señales juntas.
...
> **Ojo.** Si los dos arrays tienen **formas diferentes**, NumPy intenta una operación llamada *broadcasting*. Eso lo vamos a ver aparecer de a poco en los labs; por ahora, trabajá siempre con arrays del mismo largo y vas a estar del lado seguro.
